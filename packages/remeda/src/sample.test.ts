...
describeIterableArg("sample", ({ wrap }) => {
  describe.each([[generateRandomArray()]])("mathy stuff", (array) => {
    ...
    it("returns a random result", () => {
      ...
      for (let iteration = 0; iteration < 100; iteration++) {
        ...
        const [item] = sample(wrap(array), 1);
        collector.add(item!);
      }
  ...
  describe("identity", () => {
    it("for full (=== n) sample size", () => {
      const array = [1, 2, 3];
      const result = sample(wrap(array), 3);
    ...
    it("for large (> n) sample sizes", () => {
      const array = [1, 2, 3];
      const result = sample(wrap(array), 10);
    ...
    it("on empty arrays", () => {
      ...
      const result = sample(wrap(array), 1);
    ...
    it("on empty arrays and sample size 0", () => {
      const array: Array<number> = [];
      const result = sample(wrap(array), 0);
  ...
  describe("edge cases", () => {
    it("works on empty arrays", () => {
      const result = sample(wrap([]), 1);

      expect(result).toStrictEqual([]);
    });

    it("treats negative sample sizes as 0", () => {
      expect(sample(wrap([1, 2, 3]), -1 as number)).toStrictEqual([]);
    });

    it("rounds non-integer sample sizes", () => {
      expect(sample(wrap([1, 2, 3]), 0.5)).toHaveLength(1);
    });
  });
  ...
  describe("non-const sampleSize", () => {
    it("empty array", () => {
      const array: [] = [];
      const result = sample(wrap(array), 5 as number);
