...
describeIterableArg("sortedIndexBy", ({ wrap }) => {
  describe("runtime correctness", () => {
    test("empty array", () => {
      expect(sortedIndexBy(wrap([]), { age: 21 }, ({ age }) => age)).toBe(0);
    });

    describe("array with single item", () => {
      test("item smaller than item in array", () => {
        expect(
          sortedIndexBy(wrap([{ age: 22 }]), { age: 21 }, ({ age }) => age),
        ).toBe(0);
      });

      test("item larger than item in array", () => {
        expect(
          sortedIndexBy(wrap([{ age: 20 }]), { age: 21 }, ({ age }) => age),
        ).toBe(1);
      });

      test("item in array", () => {
        expect(
          sortedIndexBy(wrap([{ age: 21 }]), { age: 21 }, ({ age }) => age),
        ).toBe(0);
      });
    });
  ...
  function indicesSeen<T>(
    ...
    sortedIndexBy(wrap(items), item, (pivot, index) => {
      indices.push(index);
